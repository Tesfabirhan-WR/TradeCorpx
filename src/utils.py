...
import pyspark.sql
from pyspark.sql.functions import col, concat_ws, initcap, lit, trim, upper, when
...
class DATA_PATH_currency:
    ...
    def file_path(self):
        return os.getenv(
            "CURRENCY_CSV_PATH", f"{self.base_path}/country_currency.csv"
        )
...
def data_extraction():
    ...
    DATA_PATH = "/home/jovyan/src/data/azure_tradecorp_raw"
...
def spark_session(app_name="TradeCorp ETL Utils", master=None, configs=None):
    ...
    builder = (
        pyspark.sql.SparkSession.builder.appName(app_name).config(
...
def clean_orders(df):
    ...
    return (
        df.withColumn("order_date", col("order_date").cast(DateType()))
...
def clean_products(
    df_products: pyspark.sql.DataFrame,
) -> pyspark.sql.DataFrame:
    median_price = df_products.approxQuantile("unit_price", [0.5], 0.01)[0]

    return (
        df_products.fillna({"unit_price": median_price})
...
def add_sous_total(df):
    ...
    return df.withColumn(
        "sous_total",
        round(
            col("prix_unitaire") * col("quantite") * (lit(1) - col("discount")),
            2,
        ),
    )
...
def clean_customers(df_customers):
    ...
    for c, t in df_customers.dtypes:
        if t == "string":
            df_customers_trimmed = df_customers_trimmed.withColumn(
                c, trim(col(c))
            )

    df_customers_trimmed = df_customers_trimmed.withColumn(
        "contact_name", initcap(col("contact_name"))
    )
    df_customers_trimmed = df_customers_trimmed.withColumn(
        "country", upper(col("country"))
    )

    return df_customers_trimmed.dropDuplicates(["customer_id"])
...
def clean_suppliers(df):
    ...
    return (
        cleaned.withColumn("company_name", initcap(col("company_name")))
    ...
    for column_name, data_type in df.dtypes:
        if data_type == "string":
            cleaned = cleaned.withColumn(
                ...
                column_name,
                when(col(column_name) == "", lit(None)).otherwise(
                    col(column_name)
                ),
            )
...
def write_bronze(df, table_name, output_path, mode="overwrite"):
    ...
    df.write.mode(mode).parquet(f"{output_path}/{table_name}")
